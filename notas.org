...
** Implementação
   ...
   Vamos escrever a implementação de nossa tabela de dispersão em
   ~hash_table.hpp~. Começamos com algumas utilidades cujo propósito é
   detectar a /endianness/ do computador em que estamos trabalhando.
   ...
   Antes de tudo, vamos descrever os cabeçalhos que estamos
   utilizando. Usamos um ~array~ para armazenar nossas listas
   encadeadas, e utilizamos ~uint32_t~ (definido em ~cstdint~) para
   auxiliar em determinar a /endianness/ de nosso computador.

   O ~enum class Endian~ é responsável por agrupar dois símbolos, cada
   um correspondendo a uma /endianness/ diferente. A função
   ~endian_order~ retorna o símbolo apropriado, verificando se o
   primeiro byte de um inteiro de 32 bits valorado como ~1~ contém
   seus bits menos significativos (/little endian/) ou seus bits mais
   significativos (/big endian/).
   ...
   A ~struct SimpleHash~ serve como um tipo de ~HashFunction~, de
   ~Key~ para ~HashKey~, em que um valor de ~HashKey~ é obtido como
   prefiixo de um valor ~Key~, considerando a /endianness/
   utilizanda. Utilizamos ~SimpleHash~ em nossa tabela de dispersão,
   que é flexível o bastante para permitir o uso de outras
   ~HashFunction~.
   ...
   Vamos utilizar a função ~pow2~ para calcular ~2^n~, dado um
   argumento inteiro e não-negativo ~n~. Para o autor, foi
   surpreendente constatar que ~C++~ não tem uma função de
   exponenciação especializada em inteiros. Apesar de ~C++~ ter
   funções de exponenciação que lidam com pontos flutuantes, não é
   possível converter seus resultados para inteiros utilizando
   ...
   A classe ~HashTable~ representa nossa tabela de dispersão, e tem
   quatro parâmetros de template: ~Key~ e ~Val~, definindo o
   mapeamento mais relevante para o usuário; nossa chave interna
   ~HashKey~, por padrão um inteiro de oito bits sem sinal; por fim,
   ~HashFunction~, um tipo parametrizado responsável pelo mapeamento
   de ~Key~ para ~HashKey~, que por padrão é ~SimpleHash~. Note que
   ~HashTable~ instancia um objeto ~static~ ~hash_function_~ para ser
   usado como função de dispersão.

   Um outro membro estático de ~HashTable~ é ~bucket_number_~, que
   corresponde ao número de valores possíveis de ~HashKey~. Vamos
   utilizar um ``balde'' diferente para cada valor de ~HashKey~, e
   cada ``balde'' será responsável por armazenar os valores de ~Key~
   correspondentes ao seu valor de ~HashKey~, de acordo com
   ~HashFunction~. Como é descrito no código apresentado, nossos
   ``baldes'' serão listas encadeadas.
   ...
   Como membros privados, temos nosso array de baldes, com um balde
   para cada valor de ~HashFunction~, e um par de funções
   ~select_bucket_~ para encapsular o uso de ~hash_function~ no acesso
   ao balde responsável por armazenar um valor de ~Key~. Agora vamos
   descrever nossa interface pública.
   ...
   Como primeiro membro público, temos um construtor bastante simples,
   responsável apenas por inicializar nosso array de baldes. Os demais
   métodos são responsáveis por operações de busca, remoção e inserção
   na estrutura, e todos têm o mesmo funcionamento: determinam o balde
   em que deve ser feita a operação utilizando ~select_bucket_~, e
   delegam a operação para o método correspondente do balde.

** Complexidade

   TODO tem uns teoremas legais de se provar nessa parte, e como quero
   entrar em detalhes nas suas demonstrações, vou deixar para escrever
   sobre isso na próxima versão da disciplina.
...
** Árvore Geradora Mínima
   ...
   Dado um grafo $G = (V, E)$ e uma ponderação $w : E \to \mathbb{N}$
   para suas arestas, o problema da Árvore Geradora Mínima consiste em
   encontrar uma árvore geradora de $G$ com peso mínimo de acordo com
   $w$. Lembramos que uma árvore geradora de $G$ é um subgrafo conexo
   e acíclico de $G$ que contém todos os seus vértices.

   O objetivo deste laboratório é criar uma implementação do algoritmo
   de Kruskal, um algoritmo para o problema da Árvore Geradora
   Mínima. Para isso, vamos realizar as seguintes tarefas.

   1. Na classe ~WeightedGraph~, crie um construtor cujo argumento é
      uma ~std::string~ (representando o nome de um arquivo), cujo
      propósito é ler um grafo e a ponderação de suas arestas a partir
      de um arquivo. Vamos utilizar o formato ilustrado em
      ~graph-1.txt~, que consiste de uma matriz de adjacências em que
      a presença de uma aresta é representada por seu peso (positivo)
      nas posições correspondentes aos seus extremos.
   2. Crie uma função que, dado como entrada um
      ~WeightedGraph<Weight>~, retorna um
      ~std::vector<std::tuple<size_type, size_type, Weight>>~, em que
      cada tripla representa uma aresta, junto com seu peso, existente
      no grafo de entrada.
   3. Implemente uma função para ordenar o ~std::vector~ de arestas,
      em ordem crescente dos pesos das arestas. Isso pode ser feito
      com o auxílio de ~sort~, que se encontra em ~algorithm~.
   4. Vamos implementar o algoritmo de Kruskal como uma função cujo
      único argumento é um ~WeightedGraph<Weight>~: ela deve construir
      um ~std::vector~ de arestas em ordem crescente de peso; cada uma
      dessas arestas é selecionada, em ordem, se não forma ciclo junto
      com as arestas previamente selecionadas (para decidir isso,
      utilize ~DisjointSet~); por fim, as arestas selecionadas devem
      ser retornadas.
   5. Escreva um programa que recebe como argumento, na linha de
      comando, o nome de um arquivo contendo um grafo ponderado, no
      mesmo formato de ~graph-1.txt~, e imprime o peso de sua árvore
      geradora mínima, listando em seguida as arestas que fazem parte
      dela.
   6. Verifique que seu programa está correto utilizando os grafos
      contidos em ~graph-1.txt~ e ~graph-2.txt~.

** Tabelas Hash

   O problema da soma de pares é definido da seguinte maneira: dados
   $k \in \mathbb{Z}$ e um conjunto $S \subseteq \mathbb{Z}$ de $n$
   inteiros, devemos decidir se existem $s_1, s_2 \in S$ tais que
   $s_1 + s_2 = k$. Para esse problema, existe uma solução quadrática
   bastante simples, uma solução intermediária que envolve ordenação e
   uma solução linear que envolve a dispersão dos elementos de $S$.

   1. Leia a subseção 3.1 do artigo /Some Illustrative Examples on the
      Use of Hash Tables/.
   2. Nos exercícios a seguir, considere $k$ e $S$ representados por
      ~int~ e ~std::vector<int>~, respectivamente:
      1. implemente a solução de complexidade $O(n^2)$ descrita no artigo;
      2. implemente a solução de complexidade $O(n \log n)$ descrita no artigo;
      3. implemente a solução de complexidade $O(n)$ descrita no artigo.
   3. Escreva um programa que recebe uma quantidade arbitrária de
      argumentos na linha de comando: o primeiro deve representar $k$,
      e os demais devem representar elementos de $S$. O programa deve
      determinar se existem $s_1, s_2 \in S$ tais que $s_1 + s_2 = k$:
      em caso afirmativo, deve imprimir tais inteiros; caso contrário,
      deve informar que tais inteiros não existem em $S$.
