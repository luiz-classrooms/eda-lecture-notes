...
*** Complexidade
    ...
    custam $O(n)$. O acesso ao elemento mais prioritário, que tem sua
    localização bem determinada pela disposição dos nós, é feito em
    $O(1)$.
...
** Heaps Binárias
   ...
   4. Crie um construtor para inicializar ~BinaryMedianHeap~ com
      nenhum elemento. Lembre-se que o construtor deve inicializar
      os membros privados da classe.
   5. Crie um método ~std::optional<Element> median_element()~, que
      deve dar acesso ao elemento mediano, de acordo com os valores de
      prioridade. Vamos criar os métodos de inserção e remoção de
      forma que o elemento mediano seja sempre o elemento mais
      prioritário de ~BinaryMaxHeap~.
   6. Crie o método de inserção com as seguintes propriedades:
      - a inserção sempre é feita em ~BinaryMaxHeap~;
      - ~BinaryMaxHeap~ tem no máximo um elemento a mais que
        ~BinaryMinHeap~.
   7. Crie o método de remoção com as seguintes propriedades:
      - a remoção sempre é feita de ~BinaryMaxHeap~;
      - ~BinaryMaxHeap~ não tem menos elementos que ~BinaryMinHeap~.
   8. Crie um arquivo ~binary_median_heap_test.cpp~, e nele escreva
